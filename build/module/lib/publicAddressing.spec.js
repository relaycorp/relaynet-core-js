...
import { BindingType, PublicAddressingError, resolvePublicAddress, UnreachableResolverError, } from './publicAddressing';
describe('resolvePublicAddress', () => {
    ...
    test('Specified domain name should be requested', async () => {
        ...
        expect(mockGetDNS).toBeCalledWith(expect.objectContaining({ name: `_${BindingType.PDC}._tcp.${HOST}` }));
    });
    ...
    test('An error should be thrown if DNS lookup status is not NOERROR', async () => {
        ...
        await expect(resolvePublicAddress(HOST, BindingType.PDC)).rejects.toEqual(new PublicAddressingError(`SRV lookup for ${HOST_SRV_NAME} failed with status ${status}`));
    });
    test('An error should be thrown if the Answer is empty', async () => {
        mockGetDNS.mockReturnValue({ ...SUCCESSFUL_RESPONSE, answers: [] });
        await expect(resolvePublicAddress(HOST, BindingType.PDC)).rejects.toEqual(MALFORMED_ANSWER_ERROR);
    });
    ...
    test('An error should be thrown if DNSSEC verification fails', async () => {
        ...
        await expect(resolvePublicAddress(HOST, BindingType.PDC)).rejects.toEqual(new PublicAddressingError(`DNSSEC verification for SRV _${BindingType.PDC}._tcp.${HOST} failed`));
    });
