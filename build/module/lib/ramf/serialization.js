 ...
 */
export async function serialize(message, concreteMessageTypeOctet, concreteMessageVersionOctet, senderPrivateKey, signatureOptions) {
    //region Validation
    ...
    const fieldSetSerialized = makeImplicitlyTaggedSequence(new asn1js.VisibleString({ value: message.recipientAddress }), new asn1js.VisibleString({ value: message.id }), dateToASN1DateTimeInUTC(message.creationDate), new asn1js.Integer({ value: message.ttl }), new asn1js.OctetString({ valueHex: bufferToArray(message.payloadSerialized) })).toBER();
    //region Signature
...
function validateMessageLength(serialization) {
    if (MAX_RAMF_MESSAGE_LENGTH < serialization.byteLength) {
        throw new RAMFSyntaxError(`Message should not be longer than 9 MiB (got ${serialization.byteLength} octets)`);
    }
}
export async function deserialize(serialization, concreteMessageTypeOctet, concreteMessageVersionOctet, messageClass) {
...
function validateFileFormatSignature(messageFields, concreteMessageTypeOctet, concreteMessageVersionOctet) {
    ...
    if (messageFields.concreteMessageType !== concreteMessageTypeOctet) {
        ...
        throw new RAMFSyntaxError(`Expected concrete message type ${expectedMessageTypeHex} but got ${actualMessageTypeHex}`);
    }
    //endregion
    ...
    if (messageFields.concreteMessageVersion !== concreteMessageVersionOctet) {
        ...
        throw new RAMFSyntaxError(`Expected concrete message version ${expectedVersionHex} but got ${actualVersionHex}`);
    }
    //endregion
}
function validateRecipientAddress(recipientAddress) {
    try {
        ...
        if (!recipientAddress.match(PRIVATE_ADDRESS_REGEX)) {
            throw new RAMFValidationError(`Recipient address should be a valid node address (got: "${recipientAddress}")`);
        }
    }
}
function validateRecipientAddressLength(recipientAddress) {
    ...
    if (MAX_RECIPIENT_ADDRESS_LENGTH < length) {
        throw new RAMFSyntaxError(`Recipient address should not span more than ${MAX_RECIPIENT_ADDRESS_LENGTH} characters ` +
            `(got ${length})`);
    }
}
function validateMessageIdLength(messageId) {
    ...
    if (MAX_ID_LENGTH < length) {
        throw new RAMFSyntaxError(`Id should not span more than ${MAX_ID_LENGTH} characters (got ${length})`);
    }
}
