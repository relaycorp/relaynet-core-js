...
import { arrayBufferFrom, expectPkijsValuesToBeEqual, generateStubCert, getAsn1SequenceItem, getPromiseRejection, } from '../_test_utils';
...
describe('MessageSerializer', () => {
    ...
    describe('serialize', () => {
        describe('Format signature', () => {
            ...
            test('The ASCII string "Relaynet" should be at the start', async () => {
                const messageSerialized = await serialize(stubMessage, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
        ...
        describe('SignedData', () => {
            ...
            beforeAll(async () => {
                ...
                await serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
        ...
        describe('Fields', () => {
            ...
            describe('Recipient address', () => {
                test('Address should be the first item', async () => {
                    ...
                    expect(addressDeserialized.valueBlock.valueHex).toEqual(arrayBufferFrom(RECIPIENT_ADDRESS));
                });
                test('Address should not span more than 1024 characters', async () => {
                    ...
                    await expect(serialize(stubMessage, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY)).rejects.toEqual(new RAMFSyntaxError('Recipient address should not span more than 1024 characters (got 1025)'));
                });
            });
            describe('Message id', () => {
                ...
                test('Ids longer than 64 characters should be refused', async () => {
                    ...
                    await expect(serialize(stubMessage, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY)).rejects.toEqual(new RAMFSyntaxError('Id should not span more than 64 characters (got 65)'));
                });
            });
            describe('Date', () => {
                test('Date should be serialized with UTC and second-level precision', async () => {
                    ...
                    const messageSerialized = await serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
                    const fields = await deserializeFields(messageSerialized);
                    const datetimeBlock = getAsn1SequenceItem(fields, 2);
                    expect(datetimeBlock.valueBlock.valueHex).toEqual(dateToASN1DateTimeInUTC(nonUtcDate).valueBlock.valueHex);
                });
            });
            describe('TTL', () => {
                ...
                test('TTL should not be negative', async () => {
                    ...
                    await expect(serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY)).rejects.toEqual(new RAMFSyntaxError('TTL cannot be negative'));
                });
                test('TTL should not be more than 180 days', async () => {
                    const message = new StubMessage(RECIPIENT_ADDRESS, SENDER_CERTIFICATE, PAYLOAD, {
                        ttl: MAX_TTL + 1,
                    });
                    await expect(serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY)).rejects.toEqual(new RAMFSyntaxError(`TTL must be less than ${MAX_TTL} (got ${MAX_TTL + 1})`));
                });
            });
            describe('Payload', () => {
                ...
                test('Payload size should not exceed 8 MiB', async () => {
                    ...
                    const message = new StubMessage(RECIPIENT_ADDRESS, SENDER_CERTIFICATE, largePayload);
                    await expect(serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY)).rejects.toEqual(new RAMFSyntaxError(`Payload size must not exceed 8 MiB (got ${largePayload.byteLength} octets)`));
                });
            });
            async function deserializeFields(messageSerialized) {
                ...
                return derDeserialize(plaintext);
            }
        });
    });
    describe('deserialize', () => {
        ...
        test('Messages up to 9 MiB should be accepted', async () => {
            ...
            await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toThrowWithMessage(RAMFSyntaxError, 'RAMF format signature does not begin with "Relaynet"');
        });
        test('Messages larger than 9 MiB should be refused', async () => {
            ...
            await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toMatchObject({
        ...
        describe('Format signature', () => {
            test('Input should be long enough to contain format signature', async () => {
                ...
                await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toThrowWithMessage(RAMFSyntaxError, 'Serialization is too small to contain RAMF format signature');
            });
            ...
            test('A non-matching concrete message type should be refused', async () => {
                ...
                const serialization = await serialize(altMessage, stubConcreteMessageTypeOctet + 1, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
                await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toThrowWithMessage(RAMFSyntaxError, 'Expected concrete message type 0x44 but got 0x45');
            });
            test('A non-matching concrete message version should be refused', async () => {
                const altMessage = new StubMessage(RECIPIENT_ADDRESS, SENDER_CERTIFICATE, PAYLOAD);
                const serialization = await serialize(altMessage, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet + 1, SENDER_PRIVATE_KEY);
                await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toThrowWithMessage(RAMFSyntaxError, 'Expected concrete message version 0x2 but got 0x3');
            });
        });
        describe('SignedData', () => {
            test('Signature should not be accepted if invalid', async () => {
                ...
                const error = await getPromiseRejection(deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage));
            ...
            test('Sender certificate should be extracted from signature', async () => {
                const message = new StubMessage(RECIPIENT_ADDRESS, SENDER_CERTIFICATE, PAYLOAD);
                const messageSerialized = await serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
                const messageDeserialized = await deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage);
                expectPkijsValuesToBeEqual(messageDeserialized.senderCertificate.pkijsCertificate, SENDER_CERTIFICATE.pkijsCertificate);
            });
            test('Sender certificate chain should be extracted from signature', async () => {
                ...
                const messageSerialized = await serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
                const { senderCaCertificateChain } = await deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage);
        ...
        describe('Fields', () => {
            test('Fields should be DER-encoded', async () => {
                ...
                serializer.writeBuffer(Buffer.from(await cmsSignedData.sign(bufferToArray(Buffer.from('Not a DER value')), SENDER_PRIVATE_KEY, SENDER_CERTIFICATE)));
                const serialization = serializer.toBuffer();
                await expect(deserialize(bufferToArray(serialization), stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toEqual(new RAMFSyntaxError('Invalid RAMF fields'));
            });
            ...
            test('Fields sequence should not have fewer than 5 items', async () => {
                ...
                await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toEqual(new RAMFSyntaxError('Invalid RAMF fields'));
            });
            describe('Recipient address', () => {
                test('Address should be extracted', async () => {
                    ...
                    const serialization = await serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
                    const deserialization = await deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage);
                ...
                test('Address should not span more than 1024 octets', async () => {
                    ...
                    await expect(deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toEqual(new RAMFSyntaxError('Recipient address should not span more than 1024 characters (got 1025)'));
                });
                ...
                test('Invalid addresses should be refused', async () => {
                    ...
                    const serialization = await serialize(message, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, SENDER_PRIVATE_KEY);
                    await expect(deserialize(serialization, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toEqual(new RAMFSyntaxError(`Recipient address should be a valid node address (got: "${invalidAddress}")`));
                });
            });
            describe('Message id', () => {
                ...
                test('Id should not exceed 64 characters', async () => {
                    ...
                    await expect(deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toEqual(new RAMFSyntaxError('Id should not span more than 64 characters (got 65)'));
                });
            });
            describe('Date', () => {
                test('Valid date should be accepted', async () => {
                    ...
                    const message = await deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage);
                ...
                test('Date not serialized as an ASN.1 DATE-TIME should be refused', async () => {
                    ...
                    await expect(deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toMatchObject({
            ...
            describe('TTL', () => {
                test('TTL of exactly 180 days should be accepted', async () => {
                    ...
                    await expect(deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).resolves.toHaveProperty('ttl', MAX_TTL);
                });
                test('TTL greater than 180 days should not be accepted', async () => {
                    ...
                    await expect(deserialize(messageSerialized, stubConcreteMessageTypeOctet, stubConcreteMessageVersionOctet, StubMessage)).rejects.toEqual(new RAMFSyntaxError(`TTL must be less than ${MAX_TTL} (got ${MAX_TTL + 1})`));
                });
            });
            describe('Payload', () => {
        ...
        async function serializeRamfWithoutValidation(sequenceItems, senderCertificate) {
            ...
            serializer.writeUInt8(stubConcreteMessageTypeOctet);
            serializer.writeUInt8(stubConcreteMessageVersionOctet);
            const signedData = await cmsSignedData.SignedData.sign(makeImplicitlyTaggedSequence(...sequenceItems).toBER(), SENDER_PRIVATE_KEY, senderCertificate ?? SENDER_CERTIFICATE);
            serializer.writeBuffer(Buffer.from(signedData.serialize()));
