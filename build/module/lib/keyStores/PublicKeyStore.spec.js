import { derSerializePublicKey, generateECDHKeyPair, generateRSAKeyPair, getPrivateAddressFromIdentityKey, } from '../crypto_wrappers/keys';
...
describe('Identity keys', () => {
    ...
    describe('saveIdentityKey', () => {
        test('Key should be stored', async () => {
            ...
            expect(STORE.identityKeys).toHaveProperty(peerPrivateAddress, await derSerializePublicKey(publicKey));
        });
    });
    describe('retrieveIdentityKey', () => {
        test('Key should be returned if it exists', async () => {
            ...
            await expect(derSerializePublicKey(publicKeyRetrieved)).resolves.toEqual(await derSerializePublicKey(publicKey));
        });
...
describe('Session keys', () => {
    ...
    describe('retrieveLastSessionKey', () => {
        ...
        test('Retrieval errors should be wrapped', async () => {
            ...
            await expect(bogusStore.retrieveLastSessionKey(peerPrivateAddress)).rejects.toEqual(new KeyStoreError(fetchError, 'Failed to retrieve key'));
        });
    });
    describe('saveSessionKey', () => {
        ...
        test('Any error should be propagated', async () => {
            ...
            await expect(bogusStore.saveSessionKey({ keyId: sessionKeyId, publicKey: sessionPublicKey }, peerPrivateAddress, CREATION_DATE)).rejects.toEqual(new KeyStoreError('Failed to save public session key: Denied'));
        });
    });
});
