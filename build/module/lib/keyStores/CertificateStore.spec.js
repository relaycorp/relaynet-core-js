...
describe('save', () => {
    test('Expired certificate should not be saved', async () => {
        ...
        await store.save(new CertificationPath(certificate, [issuerCertificate]), subjectPrivateAddress);
    ...
    test('Expiry date should be taken from certificate', async () => {
        await store.save(certificationPath, issuerPrivateAddress);
        expect(store.dataByPrivateAddress).toHaveProperty(subjectPrivateAddress);
        expect(store.dataByPrivateAddress[subjectPrivateAddress][0].expiryDate).toEqual(setMilliseconds(subjectCertificate.expiryDate, 0));
    });
...
describe('retrieveLatest', () => {
    ...
    test('Expired certificate should be ignored', async () => {
        ...
        await store.forceSave(new CertificationPath(expiredCertificate, [issuerCertificate]), issuerPrivateAddress);
        await expect(store.retrieveLatest(subjectPrivateAddress, issuerPrivateAddress)).resolves.toBeNull();
    });
    ...
    test('Latest path should be returned', async () => {
        ...
        await store.save(new CertificationPath(olderCertificate, [issuerCertificate]), issuerPrivateAddress);
        const newerCertificate = await generateSubjectCertificate(addSeconds(now, 10));
        await store.save(new CertificationPath(newerCertificate, [issuerCertificate]), issuerPrivateAddress);
...
describe('retrieveAll', () => {
    ...
    test('Expired certificates should be ignored', async () => {
        ...
        await store.save(new CertificationPath(validCertificate, [issuerCertificate]), issuerPrivateAddress);
        const expiredCertificate = await generateSubjectCertificate(subSeconds(new Date(), 1));
        await store.forceSave(new CertificationPath(expiredCertificate, [issuerCertificate]), issuerPrivateAddress);
    ...
    test('All valid certificates should be returned', async () => {
        ...
        await store.save(new CertificationPath(certificate1, [issuerCertificate]), issuerPrivateAddress);
        const certificate2 = await generateSubjectCertificate(addSeconds(new Date(), 5));
        await store.save(new CertificationPath(certificate2, [issuerCertificate]), issuerPrivateAddress);
