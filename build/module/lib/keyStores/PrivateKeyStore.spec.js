import { derSerializePrivateKey, derSerializePublicKey, getPrivateAddressFromIdentityKey, } from '../crypto_wrappers/keys';
...
describe('Identity keys', () => {
    describe('generateIdentityKeyPair', () => {
        ...
        test('Private address should be returned', async () => {
            const keyPair = await MOCK_STORE.generateIdentityKeyPair();
            expect(keyPair.privateAddress).toEqual(await getPrivateAddressFromIdentityKey(keyPair.publicKey));
        });
        test('Public key should correspond to private key', async () => {
            ...
            await expect(derSerializePublicKey(keyPair.publicKey)).resolves.toEqual(expectedPublicKeySerialized);
        });
        ...
        test('Errors should be wrapped', async () => {
            ...
            await expect(store.generateIdentityKeyPair()).rejects.toThrowWithMessage(KeyStoreError, /^Failed to save key for \w+: Denied/);
        });
    });
});
describe('Session keys', () => {
    ...
    describe('saveSessionKey', () => {
        ...
        test('Errors should be wrapped', async () => {
            const store = new MockPrivateKeyStore(true);
            await expect(store.saveSessionKey(sessionKeyPair.privateKey, sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS)).rejects.toThrowWithMessage(KeyStoreError, `Failed to save key ${sessionKeyIdHex}: Denied`);
        });
    });
    describe('retrieveUnboundSessionKey', () => {
        test('Existing key should be returned', async () => {
            ...
            expect(await derSerializePrivateKey(keySerialized)).toEqual(await derSerializePrivateKey(sessionKeyPair.privateKey));
        });
        test('UnknownKeyError should be thrown if key id does not exist', async () => {
            await expect(MOCK_STORE.retrieveUnboundSessionKey(sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS)).rejects.toThrowWithMessage(UnknownKeyError, `Key ${sessionKeyIdHex} does not exist`);
        });
        test('Key should not be returned if owned by different node', async () => {
            await MOCK_STORE.saveSessionKey(sessionKeyPair.privateKey, sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS);
            await expect(MOCK_STORE.retrieveUnboundSessionKey(sessionKeyPair.sessionKey.keyId, `not-${PRIVATE_ADDRESS}`)).rejects.toThrowWithMessage(UnknownKeyError, 'Key is owned by a different node');
        });
        test('Subsequent session keys should not be returned', async () => {
            await MOCK_STORE.saveSessionKey(sessionKeyPair.privateKey, sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS, PEER_PRIVATE_ADDRESS);
            await expect(MOCK_STORE.retrieveUnboundSessionKey(sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS)).rejects.toThrowWithMessage(UnknownKeyError, `Key ${sessionKeyIdHex} is bound`);
        });
        test('Errors should be wrapped', async () => {
            const store = new MockPrivateKeyStore(false, true);
            await expect(store.retrieveUnboundSessionKey(sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS)).rejects.toEqual(new KeyStoreError('Failed to retrieve key: Denied'));
        });
    });
    describe('retrieveSessionKey', () => {
        test('Initial session keys should be returned', async () => {
            ...
            expect(await derSerializePrivateKey(privateKey)).toEqual(await derSerializePrivateKey(privateKey));
        });
        ...
        test('UnknownKeyError should be thrown if key pair does not exist', async () => {
            await expect(MOCK_STORE.retrieveSessionKey(sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS, PEER_PRIVATE_ADDRESS)).rejects.toThrowWithMessage(UnknownKeyError, `Key ${sessionKeyIdHex} does not exist`);
        });
        test('Key should not be returned if owned by different node', async () => {
            await MOCK_STORE.saveSessionKey(sessionKeyPair.privateKey, sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS, PEER_PRIVATE_ADDRESS);
            await expect(MOCK_STORE.retrieveSessionKey(sessionKeyPair.sessionKey.keyId, `not-${PRIVATE_ADDRESS}`, PEER_PRIVATE_ADDRESS)).rejects.toThrowWithMessage(UnknownKeyError, 'Key is owned by a different node');
        });
        test('Keys bound to another recipient should not be returned', async () => {
            ...
            await expect(MOCK_STORE.retrieveSessionKey(sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS, invalidPeerPrivateAddress)).rejects.toThrowWithMessage(UnknownKeyError, `Session key ${sessionKeyIdHex} is bound to another recipient ` +
                `(${PEER_PRIVATE_ADDRESS}, not ${invalidPeerPrivateAddress})`);
        });
        test('Errors should be wrapped', async () => {
            const store = new MockPrivateKeyStore(false, true);
            await expect(store.retrieveSessionKey(sessionKeyPair.sessionKey.keyId, PRIVATE_ADDRESS, PEER_PRIVATE_ADDRESS)).rejects.toEqual(new KeyStoreError('Failed to retrieve key: Denied'));
        });
    });
});
