...
import { derSerializePublicKey, generateRSAKeyPair, getPrivateAddressFromIdentityKey, getRSAPublicKeyFromPrivate, } from '../crypto_wrappers/keys';
...
beforeAll(async () => {
    ...
    publicGatewayCertificate = reSerializeCertificate(await issueGatewayCertificate({
    ...
    privateGatewayPrivateAddress = await getPrivateAddressFromIdentityKey(privateGatewayKeyPair.publicKey);
    privateGatewayPDCCertificate = reSerializeCertificate(await issueGatewayCertificate({
...
describe('requestPublicGatewayRegistration', () => {
    ...
    test('Registration authorization should be honoured', async () => {
        const privateGateway = new PrivateGateway(privateGatewayPrivateAddress, privateGatewayPrivateKey, KEY_STORES, {});
    ...
    test('Public key should be honoured', async () => {
        ...
        const requestSerialized = await privateGateway.requestPublicGatewayRegistration(AUTHORIZATION_SERIALIZED);
        const request = await PrivateNodeRegistrationRequest.deserialize(requestSerialized);
        await expect(derSerializePublicKey(request.privateNodePublicKey)).resolves.toEqual(await derSerializePublicKey(await getRSAPublicKeyFromPrivate(privateGatewayPrivateKey)));
    });
});
describe('savePublicGatewayChannel', () => {
    ...
    test('Registration should be refused if public gateway did not issue authorization', async () => {
        ...
        publicGatewaySessionPublicKey)).rejects.toThrowWithMessage(NodeError, 'Delivery authorization was not issued by public gateway');
    });
    ...
    test('Public key of public gateway should be stored', async () => {
        ...
        await expect(derSerializePublicKey(publicGatewayPublicKeyRetrieved)).resolves.toEqual(await derSerializePublicKey(publicGatewayPublicKey));
    });
    test('Session public key of public gateway should be stored', async () => {
        ...
        expect(keyData.publicKeyDer).toEqual(await derSerializePublicKey(publicGatewaySessionPublicKey.publicKey));
...
describe('retrievePublicGatewayChannel', () => {
    test('Null should be returned if public gateway public key is not found', async () => {
        await KEY_STORES.certificateStore.save(new CertificationPath(privateGatewayPDCCertificate, []), publicGatewayPrivateAddress);
        const privateGateway = new PrivateGateway(privateGatewayPrivateAddress, privateGatewayPrivateKey, KEY_STORES, {});
    ...
    test('Channel should be returned if it exists', async () => {
        ...
        await expect(derSerializePublicKey(channel.peerPublicKey)).resolves.toEqual(await derSerializePublicKey(publicGatewayPublicKey));
    });
    test('Crypto options should be passed', async () => {
        await KEY_STORES.certificateStore.save(new CertificationPath(privateGatewayPDCCertificate, []), publicGatewayPrivateAddress);
        await KEY_STORES.publicKeyStore.saveIdentityKey(publicGatewayPublicKey);
        const cryptoOptions = { encryption: { aesKeySize: 256 } };
        const privateGateway = new PrivateGateway(privateGatewayPrivateAddress, privateGatewayPrivateKey, KEY_STORES, cryptoOptions);
        const channel = await privateGateway.retrievePublicGatewayChannel(publicGatewayPrivateAddress, PUBLIC_GATEWAY_PUBLIC_ADDRESS);
