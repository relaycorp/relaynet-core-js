...
import { generateECDHKeyPair, generateRSAKeyPair, getPrivateAddressFromIdentityKey, } from '../../crypto_wrappers/keys';
...
describe('wrapMessagePayload', () => {
    ...
    test('There should be a session key for the recipient', async () => {
        ...
        await expect(channel.wrapMessagePayload(stubPayload)).rejects.toThrowWithMessage(NodeError, `Could not find session key for peer ${unknownPeerPrivateAddress}`);
    });
    ...
    test('Encryption options should be honoured if set', async () => {
        ...
        expect(encryptedContentInfo.contentEncryptionAlgorithm.algorithmId).toEqual(CRYPTO_OIDS.AES_CBC_192);
    });
});
