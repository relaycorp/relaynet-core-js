...
let channel;
beforeEach(() => {
    channel = new PrivatePublicGatewayChannel(privateGatewayKeyPair.privateKey, privateGatewayPDCCertificate, publicGatewayPrivateAddress, publicGatewayPublicKey, PUBLIC_GATEWAY_PUBLIC_ADDRESS, KEY_STORES, {});
});
...
describe('Endpoint registration', () => {
    ...
    describe('authorizeEndpointRegistration', () => {
        test('Gateway data should be honoured', async () => {
            ...
            const authorization = await PrivateNodeRegistrationAuthorization.deserialize(authorizationSerialized, privateGatewayKeyPair.publicKey);
    ...
    describe('verifyEndpointRegistrationAuthorization', () => {
        test('Error should be thrown if authorization is invalid', async () => {
            ...
            await expect(channel.verifyEndpointRegistrationAuthorization(authorizationSerialized)).rejects.toBeInstanceOf(InvalidMessageError);
        });
    ...
    describe('registerEndpoint', () => {
        ...
        test('Endpoint certificate should be valid for 6 months', async () => {
            ...
            const registrationSerialized = await channel.registerEndpoint(endpointPublicKey);
            const registration = await PrivateNodeRegistration.deserialize(registrationSerialized);
            expect(registration.privateNodeCertificate.expiryDate).toBeAfterOrEqualTo(addMonths(preRegistrationDate, 6));
            expect(registration.privateNodeCertificate.expiryDate).toBeBeforeOrEqualTo(addMonths(new Date(), 6));
        });
        test('Endpoint certificate should honor subject public key', async () => {
            const registrationSerialized = await channel.registerEndpoint(endpointPublicKey);
            const registration = await PrivateNodeRegistration.deserialize(registrationSerialized);
            await expect(derSerializePublicKey(await registration.privateNodeCertificate.getPublicKey())).resolves.toEqual(await derSerializePublicKey(endpointPublicKey));
        });
...
describe('generateCCA', () => {
    ...
    describe('Cargo Delivery Authorization', () => {
        test('Subject public key should be that of the public gateway', async () => {
            ...
            await expect(derSerializePublicKey(await cargoDeliveryAuthorization.getPublicKey())).resolves.toEqual(await derSerializePublicKey(await publicGatewayCertificate.getPublicKey()));
        });
