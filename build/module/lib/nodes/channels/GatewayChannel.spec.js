...
import { arrayToAsyncIterable, asyncIterableToArray, CRYPTO_OIDS, reSerializeCertificate, } from '../../_test_utils';
...
beforeAll(async () => {
    ...
    const peerCertificate = reSerializeCertificate(await issueGatewayCertificate({
...
describe('generateCargoes', () => {
    ...
    test('Recipient address should correspond to that of peer', async () => {
        ...
        const cargoesSerialized = await asyncIterableToArray(channel.generateCargoes(arrayToAsyncIterable([
            {
                expiryDate: TOMORROW,
                message: MESSAGE,
            },
    ...
    test('Encryption options should be honored if present', async () => {
        ...
        expect(await getCargoPayloadEncryptionAlgorithmId(cargoesSerialized[0])).toEqual(CRYPTO_OIDS.AES_CBC_192);
    });
    ...
    test('Signature options should be honored if present', async () => {
        ...
        await asyncIterableToArray(channel.generateCargoes(arrayToAsyncIterable([
            {
                expiryDate: TOMORROW,
                message: MESSAGE,
            },
    ...
    test('Cargo creation date should be 3 hours in the past', async () => {
        ...
        expect(cargo.creationDate.getTime()).toBeWithin(expectedCreationDate.getTime() - 5_000, expectedCreationDate.getTime() + 5_000);
    });
...
async function generateDummyParcel(recipientAddress, recipientSessionKey, finalSenderCertificate) {
    ...
    const { envelopedData } = await SessionEnvelopedData.encrypt(serviceMessageSerialized, recipientSessionKey);
...
class StubGatewayChannel extends GatewayChannel {
    static OUTBOUND_RAMF_ADDRESS = '0deadbeef';
