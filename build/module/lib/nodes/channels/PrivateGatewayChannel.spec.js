...
describe('getOrCreateCDAIssuer', () => {
    ...
    test('Subject key should be that of private gateway', async () => {
        const channel = new StubPrivateGatewayChannel();
        const issuer = await channel.getOrCreateCDAIssuer();
        await expect(derSerializePublicKey(await issuer.getPublicKey())).resolves.toEqual(await derSerializePublicKey(await getRSAPublicKeyFromPrivate(privateGatewayPrivateKey)));
    });
    ...
    async function saveCDAIssuer(cdaIssuer) {
        await KEY_STORES.certificateStore.save(new CertificationPath(cdaIssuer, []), await cdaIssuer.calculateSubjectPrivateAddress());
    }
});
describe('getCDAIssuers', () => {
    ...
    test('Other subjects should be ignored', async () => {
        ...
        await KEY_STORES.certificateStore.save(new CertificationPath(differentSubjectCertificate, []), privateGatewayPrivateAddress);
        const channel = new StubPrivateGatewayChannel();
        await expect(channel.getCDAIssuers()).resolves.toHaveLength(0);
    });
    test('Other issuers should be ignored', async () => {
        await KEY_STORES.certificateStore.save(new CertificationPath(privateGatewayPDCCertificate, []), `not-${privateGatewayPrivateAddress}`);
        const channel = new StubPrivateGatewayChannel();
        await expect(channel.getCDAIssuers()).resolves.toHaveLength(0);
    });
