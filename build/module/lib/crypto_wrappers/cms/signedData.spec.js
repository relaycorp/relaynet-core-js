...
import { arrayBufferFrom, calculateDigestHex, expectAsn1ValuesToBeEqual, expectArrayBuffersToEqual, expectPkijsValuesToBeEqual, generateStubCert, sha256Hex, } from '../../_test_utils';
...
describe('sign', () => {
    ...
    describe('SignerInfo', () => {
        ...
        test('SignerIdentifier should be IssuerAndSerialNumber', async () => {
            ...
            expectAsn1ValuesToBeEqual(signerInfo.sid.serialNumber, certificate.pkijsCertificate.serialNumber);
        });
        describe('SignedAttributes', () => {
            ...
            test('Content type attribute should be set to CMS Data', async () => {
                const signedData = await SignedData.sign(plaintext, keyPair.privateKey, certificate);
                const contentTypeAttribute = getSignerInfoAttribute(signedData.pkijsSignedData.signerInfos[0], CMS_OIDS.ATTR_CONTENT_TYPE);
                // @ts-ignore
                expect(contentTypeAttribute.values).toHaveLength(1);
                expect(
                // @ts-ignore
                contentTypeAttribute.values[0].valueBlock.toString()).toEqual(CMS_OIDS.DATA);
            });
            test('Plaintext digest should be present', async () => {
                const signedData = await SignedData.sign(plaintext, keyPair.privateKey, certificate);
                const digestAttribute = getSignerInfoAttribute(signedData.pkijsSignedData.signerInfos[0], CMS_OIDS.ATTR_DIGEST);
                // @ts-ignore
    ...
    describe('Attached certificates', () => {
        test('The signer certificate should be attached', async () => {
            ...
            expectPkijsValuesToBeEqual(signedData.pkijsSignedData.certificates[0], certificate.pkijsCertificate);
        });
        test('CA certificate chain should optionally be attached', async () => {
            ...
            expectPkijsValuesToBeEqual(attachedCertificates[1], intermediateCaCertificate.pkijsCertificate);
    ...
    describe('Hashing', () => {
        test('SHA-256 should be used by default', async () => {
            ...
            expect(
            // @ts-ignore
            Buffer.from(digestAttribute.values[0].valueBlock.valueHex).toString('hex')).toEqual(sha256Hex(plaintext));
        });
        test.each(['SHA-384', 'SHA-512'])('%s should be supported', async (hashingAlgorithmName) => {
            ...
            expect(Buffer.from(digest).toString('hex')).toEqual(calculateDigestHex(algorithmNameNodejs, plaintext));
        });
    ...
    describe('Plaintext', () => {
        test('Plaintext should be encapsulated by default', async () => {
            ...
            const plaintextOctetString = encapContentInfo.eContent.valueBlock
                .value[0];
            expectArrayBuffersToEqual(plaintextOctetString.valueBlock.valueHexView.slice().buffer, plaintext);
        });
        test('Content should not be encapsulated if requested', async () => {
            const signedData = await SignedData.sign(plaintext, keyPair.privateKey, certificate, undefined, { encapsulatePlaintext: false });
            const encapContentInfo = signedData.pkijsSignedData.encapContentInfo;
            expect(encapContentInfo).toBeInstanceOf(pkijs.EncapsulatedContentInfo);
            expect(encapContentInfo).toHaveProperty('eContentType', CMS_OIDS.DATA);
...
describe('deserialize', () => {
    test('A non-DER-encoded value should be refused', async () => {
        ...
        expect(() => SignedData.deserialize(invalidSignature)).toThrowWithMessage(CMSError, 'Could not deserialize CMS ContentInfo: Value is not DER-encoded');
    });
    test('ContentInfo wrapper should be required', async () => {
        const invalidSignature = new asn1js.Sequence().toBER(false);
        expect(() => SignedData.deserialize(invalidSignature)).toThrowWithMessage(CMSError, 'Could not deserialize CMS ContentInfo: ' +
    ...
    test('Malformed SignedData values should be refused', async () => {
        ...
        await expect(() => SignedData.deserialize(invalidSignature)).toThrowWithMessage(CMSError, 'SignedData value is malformed');
    });
...
describe('verify', () => {
    test('Value should be refused if plaintext is not encapsulated or specified', async () => {
        const signedData = await SignedData.sign(plaintext, keyPair.privateKey, certificate, undefined, {
    ...
    test('Expected plaintext should be refused if one is already encapsulated', async () => {
        const signedData = await SignedData.sign(plaintext, keyPair.privateKey, certificate);
        await expect(signedData.verify(plaintext)).rejects.toEqual(new CMSError('No specific plaintext should be expected because one is already encapsulated'));
    });
...
describe('certificates', () => {
    test('Attached CA certificates should be output', async () => {
        ...
        const signedData = await SignedData.sign(plaintext, signerKeyPair.privateKey, signerCertificate, [intermediateCaCertificate, rootCaCertificate]);
