...
export class EnvelopedData {
    ...
    static deserialize(envelopedDataSerialized) {
        ...
        if (contentInfo.contentType !== CMS_OIDS.ENVELOPED_DATA) {
            throw new CMSError(`ContentInfo does not wrap an EnvelopedData value (got OID ${contentInfo.contentType})`);
        }
        ...
        if (recipientInfosLength !== 1) {
            throw new CMSError(`EnvelopedData must have exactly one RecipientInfo (got ${recipientInfosLength})`);
        }
...
function extractOriginatorKeyId(envelopedData) {
    ...
    const matchingAttrs = unprotectedAttrs.filter((a) => a.type === RELAYNET_OIDS.ORIGINATOR_EPHEMERAL_CERT_SERIAL_NUMBER);
    ...
    if (originatorKeyIds.length !== 1) {
        throw new CMSError(`Originator key id attribute must have exactly one value (got ${originatorKeyIds.length})`);
    }
