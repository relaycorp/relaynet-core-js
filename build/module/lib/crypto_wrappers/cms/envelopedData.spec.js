...
import { arrayBufferFrom, CRYPTO_OIDS, expectAsn1ValuesToBeEqual, expectArrayBuffersToEqual, expectPkijsValuesToBeEqual, generateStubCert, getMockContext, } from '../../_test_utils';
...
import { EnvelopedData, SessionEnvelopedData, SessionlessEnvelopedData, } from './envelopedData';
...
describe('EnvelopedData', () => {
    ...
    describe('deserialize', () => {
        test('Non-DER-encoded values should be refused', () => {
            ...
            expect(() => EnvelopedData.deserialize(invalidDer)).toThrowWithMessage(CMSError, 'Could not deserialize CMS ContentInfo: Value is not DER-encoded');
        });
        test('Outer value should be a CMS ContentInfo', () => {
            ...
            expect(() => EnvelopedData.deserialize(derValue)).toThrowWithMessage(CMSError, /^Could not deserialize CMS ContentInfo: /);
        });
        test('OID of inner value should be that of EnvelopedData', async () => {
            ...
            expect(() => EnvelopedData.deserialize(contentInfoSerialized)).toThrowWithMessage(CMSError, 'ContentInfo does not wrap an EnvelopedData value (got OID 1.2)');
        });
        test('Inner value should be a valid EnvelopedData', async () => {
            ...
            const contentInfoSerialized = contentInfo.toSchema().toBER(false);
            expect(() => EnvelopedData.deserialize(contentInfoSerialized)).toThrowWithMessage(CMSError, /^Invalid EnvelopedData value: /);
        });
        ...
        test('An EnvelopedData with multiple recipientInfos should be refused', async () => {
            ...
            const contentInfoSerialized = contentInfo.toSchema().toBER(false);
            expect(() => EnvelopedData.deserialize(contentInfoSerialized)).toThrowWithMessage(CMSError, 'EnvelopedData must have exactly one RecipientInfo (got 2)');
        });
        test('KeyTransRecipientInfo should result in SessionlessEnvelopedData instance', async () => {
            const originalEnvelopedData = await SessionlessEnvelopedData.encrypt(plaintext, nodeCertificate);
            ...
            expectAsn1ValuesToBeEqual(originalEnvelopedData.pkijsEnvelopedData.toSchema(), envelopedData.pkijsEnvelopedData.toSchema());
        });
        test('A KeyAgreeRecipientInfo should result in a SessionEnvelopedData instance', async () => {
            ...
            expectAsn1ValuesToBeEqual(envelopedDataDeserialized.pkijsEnvelopedData.toSchema(), envelopedData.pkijsEnvelopedData.toSchema());
        });
        test('An unsupported RecipientInfo should be refused', async () => {
            const unsupportedEnvelopedData = await SessionlessEnvelopedData.encrypt(plaintext, nodeCertificate);
            ...
            expect(() => EnvelopedData.deserialize(unsupportedEnvelopedDataSerialized)).toThrowWithMessage(CMSError, 'Unsupported RecipientInfo (variant: 3)');
        });
    });
});
    ...
    describe('encrypt', () => {
        describe('RecipientInfo', () => {
            ...
            test('KeyTransRecipientInfo should use issuerAndSerialNumber choice', async () => {
                ...
                assertPkiType(keyTransRecipientInfo.rid, pkijs.IssuerAndSerialNumber, 'keyTransRecipientInfo.rid');
                ...
                expectPkijsValuesToBeEqual(keyTransRecipientInfo.rid.issuer, nodeCertificate.pkijsCertificate.issuer);
                expectAsn1ValuesToBeEqual(keyTransRecipientInfo.rid.serialNumber, nodeCertificate.pkijsCertificate.serialNumber);
            });
        ...
        describeEncryptedContentInfoEncryption(async (options) => {
            const envelopedData = await SessionlessEnvelopedData.encrypt(plaintext, nodeCertificate, options);
...
describe('SessionEnvelopedData', () => {
    describe('encrypt', () => {
        ...
        test('Generated (EC)DH key id should be output and included in unprotectedAttrs', async () => {
            const { envelopedData, dhKeyId } = await SessionEnvelopedData.encrypt(plaintext, bobSessionKey);
        ...
        describeEncryptedContentInfoEncryption(async (options) => {
            const { envelopedData } = await SessionEnvelopedData.encrypt(plaintext, bobSessionKey, options);
            return envelopedData.serialize();
        });
    });
    describe('getOriginatorKey', () => {
        ...
        describe('keyId', () => {
            ...
            test('Call should fail if unprotectedAttrs is missing', async () => {
                ...
                await expect(envelopedData.getOriginatorKey()).rejects.toEqual(new CMSError('unprotectedAttrs must be present when using channel session'));
            });
            ...
            test('Call should fail if originator key id is missing', async () => {
                ...
                await expect(envelopedData.getOriginatorKey()).rejects.toEqual(new CMSError('unprotectedAttrs does not contain originator key id'));
            });
            test('Call should fail if attribute for originator key id is empty', async () => {
                ...
                await expect(envelopedData.getOriginatorKey()).rejects.toEqual(new CMSError('Originator key id attribute must have exactly one value (got 0)'));
            });
            test('Call should fail if attribute for originator key id is multi-valued', async () => {
                ...
                envelopedData.pkijsEnvelopedData.unprotectedAttrs = [invalidAttribute];
                await expect(envelopedData.getOriginatorKey()).rejects.toEqual(new CMSError('Originator key id attribute must have exactly one value (got 2)'));
            });
        });
        describe('publicKey', () => {
            ...
            test('Call should fail if RecipientInfo is not KeyAgreeRecipientInfo', async () => {
                ...
                await expect(envelopedData.getOriginatorKey()).rejects.toEqual(new CMSError('Expected KeyAgreeRecipientInfo (got variant: 3)'));
            });
        });
    });
    test('getRecipientKeyId() should return the recipient key id', async () => {
...
function describeEncryptedContentInfoEncryption(encryptFunc) {
    describe('EncryptedContentInfo', () => {
        test('AES-CBC-128 should be used by default', async () => {
            ...
            expect(envelopedData.encryptedContentInfo.contentEncryptionAlgorithm.algorithmId).toEqual(CRYPTO_OIDS.AES_CBC_128);
        });
        test.each([
        ...
        test('Key sizes other than 128, 192 and 256 should be refused', async () => {
            await expect(encryptFunc({ aesKeySize: 512 })).rejects.toEqual(new CMSError('Invalid AES key size (512)'));
        });
    });
}
