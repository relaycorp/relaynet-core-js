...
import { derDeserializeECDHPrivateKey, derDeserializeECDHPublicKey, derDeserializeRSAPrivateKey, derDeserializeRSAPublicKey, derSerializePrivateKey, derSerializePublicKey, generateECDHKeyPair, generateRSAKeyPair, getPrivateAddressFromIdentityKey, getPublicKeyDigest, getPublicKeyDigestHex, getRSAPublicKeyFromPrivate, } from './keys';
describe('generateRsaKeyPair', () => {
    ...
    describe('Modulus', () => {
        ...
        test('Modulus < 2048 should not supported', async () => {
            await expect(generateRSAKeyPair({ modulus: 1024 })).rejects.toThrow('RSA modulus must be => 2048 per RS-018 (got 1024)');
        });
    });
    describe('Hashing algorithm', () => {
        ...
        test('SHA-1 should not be supported', async () => {
            await expect(generateRSAKeyPair({ hashingAlgorithm: 'SHA-1' })).rejects.toThrow('SHA-1 is disallowed by RS-018');
        });
    });
});
describe('generateDHKeyPair', () => {
    ...
    test.each([['P-384', 'P-521']])('%s should also be supported', async (curveName) => {
        const keyPair = await generateECDHKeyPair(curveName);
...
describe('getRSAPublicKeyFromPrivate', () => {
    test('Public key should be returned', async () => {
        ...
        await expect(derSerializePublicKey(publicKey)).resolves.toEqual(await derSerializePublicKey(keyPair.publicKey));
    });
...
describe('Key deserializers', () => {
    ...
    test('derDeserializeRSAPublicKey should convert DER public key to RSA key', async () => {
        ...
        expect(mockImportKey).toBeCalledWith('spki', bufferToArray(stubKeyDer), rsaAlgorithmOptions, true, ['verify']);
    });
    ...
    test('derDeserializeRSAPrivateKey should convert DER private key to RSA key', async () => {
        ...
        expect(mockImportKey).toBeCalledTimes(1);
        expect(mockImportKey).toBeCalledWith('pkcs8', bufferToArray(stubKeyDer), rsaAlgorithmOptions, true, ['sign']);
    });
    ...
    test('derDeserializeECDHPublicKey should convert DER public key to ECDH key', async () => {
        ...
        expect(publicKey).toBe(stubKeyPair.publicKey);
        expect(mockImportKey).toBeCalledTimes(1);
        expect(mockImportKey).toBeCalledWith('spki', bufferToArray(stubKeyDer), { name: 'ECDH', namedCurve: ecdhCurveName }, true, []);
    });
    ...
    test('derDeserializeECDHPrivateKey should convert DER private key to ECDH key', async () => {
        ...
        expect(privateKey).toBe(stubKeyPair.privateKey);
        expect(mockImportKey).toBeCalledTimes(1);
        expect(mockImportKey).toBeCalledWith('pkcs8', bufferToArray(stubKeyDer), { name: 'ECDH', namedCurve: ecdhCurveName }, true, ['deriveBits', 'deriveKey']);
    });
...
test('getPublicKeyDigest should return the SHA-256 digest of the public key', async () => {
    ...
    expect(Buffer.from(digest)).toEqual(createHash('sha256')
...
describe('getPrivateAddressFromIdentityKey', () => {
    ...
    test('DH keys should be refused', async () => {
        const keyPair = await generateECDHKeyPair();
        await expect(getPrivateAddressFromIdentityKey(keyPair.publicKey)).rejects.toThrowWithMessage(Error, 'Only RSA keys are supported (got ECDH)');
    });
});
