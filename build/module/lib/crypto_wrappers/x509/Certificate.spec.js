...
import { derSerializePublicKey, generateRSAKeyPair, getPrivateAddressFromIdentityKey, } from '../keys';
...
describe('deserialize()', () => {
    ...
    test('should error out with invalid DER values', () => {
        ...
        expect(() => Certificate.deserialize(invalidDer)).toThrowWithMessage(Error, 'Value is not DER-encoded');
    });
});
describe('issue()', () => {
    ...
    test('should refuse start date if after expiry date of issuer', async () => {
        ...
        await expect(Certificate.issue({
            ...baseCertificateOptions,
            issuerCertificate,
            issuerPrivateKey: subjectKeyPair.privateKey,
            subjectPublicKey: subjectKeyPair.publicKey,
    ...
    test('should refuse an issuer certificate without extensions', async () => {
        ...
        await expect(Certificate.issue({
            ...
            issuerPrivateKey: issuerKeyPair.privateKey,
            subjectPublicKey: subjectKeyPair.publicKey,
        })).rejects.toEqual(new CertificateError('Basic constraints extension is missing from issuer certificate'));
    });
...
describe('getCommonName()', () => {
    ...
    test('should error out when the address is not found', async () => {
        ...
        expect(() => cert.getCommonName()).toThrowWithMessage(CertificateError, 'Distinguished Name does not contain Common Name');
    });
});
describe('calculateSubjectPrivateAddress', () => {
    test('Private node address should be returned', async () => {
        ...
        await expect(nodeCertificate.calculateSubjectPrivateAddress()).resolves.toEqual(await getPrivateAddressFromIdentityKey(nodeKeyPair.publicKey));
    });
...
describe('validate()', () => {
    ...
    test('Certificate version other than 3 should be refused', async () => {
        ...
        expect(() => cert.validate()).toThrowWithMessage(CertificateError, 'Only X.509 v3 certificates are supported (got v2)');
    });
    test('Certificate not yet valid should not be accepted', async () => {
        ...
        expect(() => cert.validate()).toThrowWithMessage(CertificateError, 'Certificate is not yet valid');
    });
    test('Expired certificate should not be accepted', async () => {
        ...
        const cert = await generateStubCert({ attributes: { validityEndDate, validityStartDate } });
        expect(() => cert.validate()).toThrowWithMessage(CertificateError, 'Certificate already expired');
    });
});
describe('getCertificationPath', () => {
    ...
    beforeAll(async () => {
        ...
        stubRootCa = reSerializeCertificate(await generateStubCert({
    ...
    test('Cert issued by trusted cert should be trusted', async () => {
        const cert = reSerializeCertificate(await generateStubCert({
    ...
    test('Cert not issued by trusted cert should not be trusted', async () => {
        const cert = await generateStubCert();
        await expect(cert.getCertificationPath([], [stubRootCa])).rejects.toEqual(new CertificateError('No valid certificate paths found'));
    });
    ...
    test('Cert issued by untrusted intermediate should be trusted if root is trusted', async () => {
        ...
        const intermediateCaCert = reSerializeCertificate(await generateStubCert({
            attributes: { isCA: true },
            issuerCertificate: stubRootCa,
            issuerPrivateKey: stubTrustedCaPrivateKey,
    ...
    test('Cert issued by untrusted intermediate CA should not be trusted', async () => {
        ...
        await expect(cert.getCertificationPath([reSerializeCertificate(untrustedIntermediateCaCert)], [stubRootCa])).rejects.toEqual(new CertificateError('No valid certificate paths found'));
    });
    test('Including trusted intermediate CA should not make certificate trusted', async () => {
        ...
        const cert = await generateStubCert();
        await expect(cert.getCertificationPath([trustedIntermediateCaCert], [stubRootCa])).rejects.toEqual(new CertificateError('No valid certificate paths found'));
    });
...
test('getPublicKey should return the subject public key', async () => {
    ...
    await expect(derSerializePublicKey(publicKey)).resolves.toEqual(await derSerializePublicKey(subjectKeyPair.publicKey));
});
