...
describe('serialize', () => {
    test('Leaf certificate should be serialized', () => {
        ...
        expectArrayBuffersToEqual(leafCertificateASN1.valueBlock.toBER(), subjectCertificate.serialize());
    });
...
describe('deserialize', () => {
    test('Serialization should contain a sequence of a least 2 items', async () => {
        const invalidSerialization = makeImplicitlyTaggedSequence(new OctetString()).toBER(false);
        expect(() => CertificationPath.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Serialization did not meet structure of a CertificationPath');
    });
    test('Malformed subject certificate should be refused', async () => {
        const invalidSerialization = makeImplicitlyTaggedSequence(new VisibleString({ value: 'This is supposed to be a cert' }), new Set()).toBER(false);
        expect(() => CertificationPath.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Leaf certificate is malformed');
    });
    test('Malformed chain should be refused', async () => {
        const invalidSerialization = makeImplicitlyTaggedSequence(new OctetString({ valueHex: subjectCertificate.serialize() }), new Integer({ value: 42 })).toBER(false);
        expect(() => CertificationPath.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Serialization did not meet structure of a CertificationPath');
    });
    test('Malformed chain certificate should be refused', async () => {
        const invalidSerialization = makeImplicitlyTaggedSequence(new OctetString({ valueHex: subjectCertificate.serialize() }), makeImplicitlyTaggedSequence(new VisibleString({ value: 'This is a "certificate" ;-)' }))).toBER(false);
        expect(() => CertificationPath.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Certificate authorities contain malformed certificate');
    });
