import { ObjectIdentifier, OctetString, Sequence, VisibleString } from 'asn1js';
...
describe('PrivateNodeRegistrationAuthorization', () => {
    ...
    describe('serialize', () => {
        ...
        test('Expiry date should be honored', async () => {
            ...
            expect(expiryDateASN1.valueBlock.valueHex).toEqual(dateToASN1DateTimeInUTC(expiryDate).valueBlock.valueHex);
        });
        ...
        test('Signature should be valid', async () => {
            ...
            const expectedPlaintext = makeImplicitlyTaggedSequence(new ObjectIdentifier({ value: RELAYNET_OIDS.NODE_REGISTRATION.AUTHORIZATION }), dateToASN1DateTimeInUTC(expiryDate), new OctetString({ valueHex: gatewayData })).toBER();
            await expect(verify(signature, gatewayKeyPair.publicKey, expectedPlaintext)).resolves.toBeTrue();
        });
    });
    describe('deserialize', () => {
        test('Malformed values should be refused', async () => {
            await expect(PrivateNodeRegistrationAuthorization.deserialize(arrayBufferFrom('foo'), gatewayKeyPair.publicKey)).rejects.toEqual(new InvalidMessageError('Serialization is not a valid PrivateNodeRegistrationAuthorization'));
        });
        test('Sequence should have at least 3 items', async () => {
            const serialization = makeImplicitlyTaggedSequence(new VisibleString({ value: 'foo' }), new VisibleString({ value: 'bar' })).toBER();
            await expect(PrivateNodeRegistrationAuthorization.deserialize(serialization, gatewayKeyPair.publicKey)).rejects.toEqual(new InvalidMessageError('Serialization is not a valid PrivateNodeRegistrationAuthorization'));
        });
        test('Expired authorizations should be refused', async () => {
            ...
            const serialization = await authorization.serialize(gatewayKeyPair.privateKey);
            await expect(PrivateNodeRegistrationAuthorization.deserialize(serialization, gatewayKeyPair.publicKey)).rejects.toEqual(new InvalidMessageError('Authorization already expired'));
        });
        test('Invalid signatures should be refused', async () => {
            const tomorrow = moment().add(1, 'days').toDate();
            const serialization = makeImplicitlyTaggedSequence(dateToASN1DateTimeInUTC(tomorrow), new VisibleString({ value: 'gateway data' }), new VisibleString({ value: 'invalid signature' })).toBER();
            await expect(PrivateNodeRegistrationAuthorization.deserialize(serialization, gatewayKeyPair.publicKey)).rejects.toEqual(new InvalidMessageError('Authorization signature is invalid'));
        });
        test('Valid values should be accepted', async () => {
            const authorization = new PrivateNodeRegistrationAuthorization(expiryDate, gatewayData);
            const serialization = await authorization.serialize(gatewayKeyPair.privateKey);
            const authorizationDeserialized = await PrivateNodeRegistrationAuthorization.deserialize(serialization, gatewayKeyPair.publicKey);
