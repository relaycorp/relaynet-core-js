import { arrayBufferFrom, expectArrayBuffersToEqual, generateStubCert, reSerializeCertificate, } from '../../_test_utils';
import { generateRSAKeyPair } from '../../crypto_wrappers/keys';
import InvalidMessageError from '../../messages/InvalidMessageError';
import Parcel from '../../messages/Parcel';
import { issueDeliveryAuthorization, issueEndpointCertificate, issueGatewayCertificate, } from '../../pki/issuance';
...
beforeAll(async () => {
    ...
    gatewayCertificate = reSerializeCertificate(await issueGatewayCertificate({
    ...
    recipientCertificate = reSerializeCertificate(await issueEndpointCertificate({
    ...
    pdaCertificate = reSerializeCertificate(await issueDeliveryAuthorization({
...
describe('deserializeAndValidateParcel', () => {
    test('Malformed parcels should be refused', async () => {
        const collection = new ParcelCollection(arrayBufferFrom('invalid'), [gatewayCertificate], jest.fn());
    ...
    test('Parcels bound for public endpoints should be refused', async () => {
        ...
        const collection = new ParcelCollection(await parcel.serialize(pdaGranteeKeyPair.privateKey), [gatewayCertificate], jest.fn());
        await expect(collection.deserializeAndValidateParcel()).rejects.toBeInstanceOf(InvalidMessageError);
    });
    ...
    test('Valid parcels should be returned', async () => {
        const parcel = new Parcel(await recipientCertificate.calculateSubjectPrivateAddress(), pdaCertificate, Buffer.from([]), { senderCaCertificateChain: [recipientCertificate] });
        const collection = new ParcelCollection(await parcel.serialize(pdaGranteeKeyPair.privateKey), [gatewayCertificate], jest.fn());
