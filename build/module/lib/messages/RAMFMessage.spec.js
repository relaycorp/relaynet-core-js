...
import { SessionEnvelopedData, SessionlessEnvelopedData, } from '../crypto_wrappers/cms/envelopedData';
...
describe('RAMFMessage', () => {
    ...
    beforeAll(async () => {
        ...
        senderCertificate = reSerializeCertificate(await generateStubCert({
    ...
    describe('constructor', () => {
        describe('Id', () => {
            test('Id should fall back to UUID4 when left unspecified', () => {
                const message = new StubMessage(recipientPrivateAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT);
        ...
        describe('Date', () => {
            ...
            test('A custom date should be accepted', () => {
                ...
                const message = new StubMessage(recipientPrivateAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT, { creationDate: date });
        ...
        describe('TTL', () => {
            ...
            test('A custom TTL under 2^24 should be accepted', () => {
                const ttl = 2 ** 24 - 1;
                const message = new StubMessage(recipientPrivateAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT, { ttl });
        ...
        describe('Sender CA certificate chain', () => {
            ...
            test('A custom sender certificate chain should be accepted', async () => {
                const chain = [await generateStubCert(), await generateStubCert()];
                const message = new StubMessage(recipientPrivateAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT, { senderCaCertificateChain: chain });
            ...
            test('Sender certificate should be excluded from chain if included', async () => {
                const chain = [await generateStubCert()];
                const message = new StubMessage(recipientPrivateAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT, {
    ...
    test('getSenderCertificationPath should return certification path', async () => {
        const message = new StubMessage(await stubSenderChain.recipientCert.calculateSubjectPrivateAddress(), stubSenderChain.senderCert, STUB_PAYLOAD_PLAINTEXT, {
    ...
    test('expiryDate field should calculate expiry date from creation date and TTL', () => {
        const message = new StubMessage('some-address', stubSenderChain.senderCert, STUB_PAYLOAD_PLAINTEXT, {
    ...
    describe('validate', () => {
        describe('Recipient address', () => {
            ...
            test('Syntactically-invalid addresses should be refused', async () => {
                const message = new StubMessage('this is an invalid address', senderCertificate, STUB_PAYLOAD_PLAINTEXT);
        ...
        describe('Authorization without trusted certificates', () => {
            test('Invalid sender certificate should be refused', async () => {
                ...
                const message = new StubMessage('0deadbeef', invalidSenderCertificate, STUB_PAYLOAD_PLAINTEXT);
        ...
        describe('Authorization with trusted certificates', () => {
            test('Message should be refused if sender is not trusted', async () => {
                const message = new StubMessage(await stubSenderChain.recipientCert.calculateSubjectPrivateAddress(), senderCertificate, STUB_PAYLOAD_PLAINTEXT);
                jestDateMock.advanceBy(1_000);
                await expect(message.validate(undefined, [stubSenderChain.rootCert])).rejects.toEqual(new InvalidMessageError('Sender is not authorized: No valid certificate paths found'));
            });
            ...
            test('Message should be refused if recipient is private and did not authorize', async () => {
                const message = new StubMessage('0deadbeef', stubSenderChain.senderCert, STUB_PAYLOAD_PLAINTEXT, {
                    senderCaCertificateChain: [stubSenderChain.recipientCert],
                });
                jestDateMock.advanceBy(1_000);
                await expect(message.validate(undefined, [stubSenderChain.rootCert])).rejects.toEqual(new InvalidMessageError(`Sender is not authorized to reach ${message.recipientAddress}`));
            });
            test('Message should be accepted if recipient address is public', async () => {
                const message = new StubMessage('https://example.com', stubSenderChain.senderCert, STUB_PAYLOAD_PLAINTEXT, {
                    senderCaCertificateChain: [stubSenderChain.recipientCert],
                });
                jestDateMock.advanceBy(1_000);
                const certificationPath = await message.validate(undefined, [stubSenderChain.rootCert]);
        ...
        describe('Validity period', () => {
            ...
            test('Date equal to the current date should be accepted', async () => {
                const stubDate = new Date(senderCertificate.pkijsCertificate.notAfter.value.getTime() - 1_000);
                stubDate.setSeconds(0, 0);
                const message = new StubMessage(recipientPublicAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT, { creationDate: stubDate });
                jestDateMock.advanceTo(stubDate);
                await message.validate();
            });
            test('Date should not be in the future', async () => {
                const message = new StubMessage(recipientPublicAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT);
                ...
                await expect(message.validate()).rejects.toEqual(new InvalidMessageError('Message date is in the future'));
            });
            ...
            test('TTL in the past should not be accepted', async () => {
                const message = new StubMessage(recipientPublicAddress, senderCertificate, STUB_PAYLOAD_PLAINTEXT, { ttl: 1 });
                jestDateMock.advanceTo(message.creationDate.getTime() + (message.ttl + 1) * 1_000);
                await expect(message.validate()).rejects.toEqual(new InvalidMessageError('Message already expired'));
            });
        });
    });
    describe('unwrapPayload', () => {
        test('SessionlessEnvelopedData payload should be unsupported', async () => {
            const envelopedData = await SessionlessEnvelopedData.encrypt(STUB_PAYLOAD_PLAINTEXT, recipientCertificate);
            const recipientKeyStore = new MockPrivateKeyStore();
            const stubMessage = new StubMessage(recipientPrivateAddress, senderCertificate, Buffer.from(envelopedData.serialize()));
            await expect(stubMessage.unwrapPayload(recipientKeyStore)).rejects.toThrowWithMessage(RAMFError, 'Sessionless payloads are no longer supported');
        });
        test('Payload for private recipient should be decrypted with key store', async () => {
            ...
            const { envelopedData } = await SessionEnvelopedData.encrypt(STUB_PAYLOAD_PLAINTEXT, recipientSessionKeyPair.sessionKey);
            const stubMessage = new StubMessage(recipientPrivateAddress, senderCertificate, Buffer.from(envelopedData.serialize()));
            const recipientKeyStore = new MockPrivateKeyStore();
        ...
        test('Payload for public recipient should be decrypted with key store', async () => {
            const recipientSessionKeyPair = await SessionKeyPair.generate();
            const { envelopedData } = await SessionEnvelopedData.encrypt(STUB_PAYLOAD_PLAINTEXT, recipientSessionKeyPair.sessionKey);
            const stubMessage = new StubMessage('https://example.com', senderCertificate, Buffer.from(envelopedData.serialize()));
            const recipientKeyStore = new MockPrivateKeyStore();
        ...
        test('Recipient private address should be passed if only public is available', async () => {
            const stubMessage = new StubMessage('https://example.com', senderCertificate, Buffer.from([]));
            const recipientKeyStore = new MockPrivateKeyStore();
            await expect(stubMessage.unwrapPayload(recipientKeyStore)).rejects.toThrowWithMessage(RAMFError, 'Recipient private address should be passed because message uses public address');
        });
        test('Payload for private recipient should be decrypted with private key', async () => {
            const recipientSessionKeyPair = await SessionKeyPair.generate();
            const { envelopedData } = await SessionEnvelopedData.encrypt(STUB_PAYLOAD_PLAINTEXT, recipientSessionKeyPair.sessionKey);
            const stubMessage = new StubMessage('0123', senderCertificate, Buffer.from(envelopedData.serialize()));
    ...
    describe('isRecipientAddressPrivate', () => {
        ...
        test('False should be returned when address is public', () => {
            const message = new StubMessage('https://example.com', senderCertificate, STUB_PAYLOAD_PLAINTEXT);
...
async function generateAuthorizedSenderChain() {
    ...
    const rootCert = reSerializeCertificate(await generateStubCert({
        attributes: { isCA: true },
        issuerPrivateKey: rootKeyPair.privateKey,
        subjectPublicKey: rootKeyPair.publicKey,
    }));
    const recipientKeyPair = await generateRSAKeyPair();
    const recipientCert = reSerializeCertificate(await generateStubCert({
        attributes: { isCA: true },
        issuerCertificate: rootCert,
        issuerPrivateKey: rootKeyPair.privateKey,
        subjectPublicKey: recipientKeyPair.publicKey,
    }));
    const senderCert = reSerializeCertificate(await generateStubCert({
