...
describe('ParcelCollectionAck', () => {
    ...
    describe('serialize', () => {
        test('Serialization should start with format signature', () => {
            const pca = new ParcelCollectionAck(SENDER_ENDPOINT_PRIVATE_ADDRESS, RECIPIENT_ENDPOINT_ADDRESS, PARCEL_ID);
        ...
        test('An ACK should be serialized as a 3-item sequence', () => {
            ...
            expect(pcaSequenceItems[0].valueBlock.valueHex).toEqual(arrayBufferFrom(SENDER_ENDPOINT_PRIVATE_ADDRESS));
            expect(pcaSequenceItems[1].valueBlock.valueHex).toEqual(arrayBufferFrom(RECIPIENT_ENDPOINT_ADDRESS));
            expect(pcaSequenceItems[2].valueBlock.valueHex).toEqual(arrayBufferFrom(PARCEL_ID));
        });
        ...
        function parsePCA(pcaSerialized) {
            ...
            return derDeserialize(derSequenceSerialized);
        }
    });
    describe('deserialize', () => {
        test('Serialization should start with format signature', () => {
            const invalidSerialization = arrayBufferFrom('RelaynetA0');
            expect(() => ParcelCollectionAck.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Format signature should be that of a PCA');
        });
        test('ACK should be refused if it has fewer than 3 items', () => {
            ...
            expect(() => ParcelCollectionAck.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'PCA did not meet required structure');
        });
