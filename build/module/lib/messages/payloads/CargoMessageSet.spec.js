...
import { arrayBufferFrom, arrayToAsyncIterable, asyncIterableToArray, expectArrayBuffersToEqual, generateStubCert, } from '../../_test_utils';
...
describe('CargoMessageSet', () => {
    describe('deserialize', () => {
        test('Non-DER-encoded values should be refused', () => {
            ...
            expect(() => CargoMessageSet.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Serialization is not a valid CargoMessageSet');
        });
        ...
        test('A multi-item sequence should be accepted', () => {
            const messages = [STUB_MESSAGE, arrayBufferFrom('another message')];
            const asn1Sequence = new asn1js.Sequence();
    ...
    describe('deserializeItem', () => {
        ...
        test('An error should be thrown when non-RAMF message is found', async () => {
            ...
            await expect(CargoMessageSet.deserializeItem(invalidItemSerialized)).rejects.toThrow(/Value is not a valid Cargo Message Set item/);
        });
        test('An error should be yielded when unsupported RAMF message is found', async () => {
            ...
            await expect(CargoMessageSet.deserializeItem(cargoSerialization)).rejects.toThrow(/Value is not a valid Cargo Message Set item/);
        });
    });
    describe('batchMessagesSerialized', () => {
        ...
        test('Messages exceeding the max per-message size should be refused', async () => {
            ...
            const messages = arrayToAsyncIterable([{ messageSerialized, expiryDate: EXPIRY_DATE }]);
            await expect(asyncIterableToArray(CargoMessageSet.batchMessagesSerialized(messages))).rejects.toEqual(new InvalidMessageError(`Cargo messages must not exceed ${CargoMessageSet.MAX_MESSAGE_LENGTH} octets ` +
        ...
        test('Messages collectively reaching max length should be placed together', async () => {
            const messageSerialized1 = arrayBufferFrom('a'.repeat(CargoMessageSet.MAX_MESSAGE_LENGTH / 2 - 3));
            const messageSerialized2 = arrayBufferFrom('a'.repeat(CargoMessageSet.MAX_MESSAGE_LENGTH / 2 - 2));
            const messages = arrayToAsyncIterable([
    ...
    describe('serialize', () => {
        ...
        test('A one-item array should serialized as such', () => {
            ...
            expectArrayBuffersToEqual(stubMessageAsn1.valueBlock.valueHex, STUB_MESSAGE);
        });
        test('A multi-item array should serialized as such', () => {
            const stubMessages = [STUB_MESSAGE, arrayBufferFrom('bye')];
            ...
            for (let index = 0; index < stubMessages.length; index++) {
                ...
                expectArrayBuffersToEqual(messageAsn1.valueBlock.valueHex, stubMessages[index]);
            }
        });
    });
});
