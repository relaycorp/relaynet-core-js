...
import { arrayBufferFrom, expectArrayBuffersToEqual, generateStubCert, getAsn1SequenceItem, } from '../../_test_utils';
...
describe('deserialize', () => {
    test('Malformed sequences should be refused', () => {
        expect(() => CargoCollectionRequest.deserialize(arrayBufferFrom('malformed'))).toThrowWithMessage(InvalidMessageError, 'Serialization is not a valid CargoCollectionRequest');
    });
    test('Sequence should have at least one item', () => {
        expect(() => CargoCollectionRequest.deserialize(makeImplicitlyTaggedSequence().toBER())).toThrowWithMessage(InvalidMessageError, 'Serialization is not a valid CargoCollectionRequest');
    });
    test('Malformed Cargo Delivery Authorizations should be refused', () => {
        const invalidCertificate = new Integer({ value: 42 });
        expect(() => CargoCollectionRequest.deserialize(makeImplicitlyTaggedSequence(invalidCertificate).toBER())).toThrowWithMessage(InvalidMessageError, /^CargoCollectionRequest contains a malformed Cargo Delivery Authorization: /);
    });
