...
describe('ServiceMessage', () => {
    ...
    describe('deserialize', () => {
        test('Serialization should be DER sequence', () => {
            ...
            expect(() => ServiceMessage.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Invalid service message serialization');
        });
        test('Sequence should have at least two items', () => {
            const invalidSerialization = makeImplicitlyTaggedSequence(new asn1js.VisibleString({ value: 'foo' })).toBER();
            expect(() => ServiceMessage.deserialize(invalidSerialization)).toThrowWithMessage(InvalidMessageError, 'Invalid service message serialization');
        });
