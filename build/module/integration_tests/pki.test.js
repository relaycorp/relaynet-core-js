import { generateRSAKeyPair, issueDeliveryAuthorization, issueEndpointCertificate, issueGatewayCertificate, Parcel, } from '..';
...
beforeAll(async () => {
    ...
    publicGatewayCert = reSerializeCertificate(await issueGatewayCertificate({
    ...
    privateGatewayCert = reSerializeCertificate(await issueGatewayCertificate({
    ...
    peerEndpointCert = reSerializeCertificate(await issueEndpointCertificate({
    ...
    endpointPdaCert = reSerializeCertificate(await issueDeliveryAuthorization({
...
test('Messages by authorized senders should be accepted', async () => {
    const parcel = new Parcel(await peerEndpointCert.calculateSubjectPrivateAddress(), endpointPdaCert, Buffer.from('hey'), {
...
test('Certificate chain should be computed corrected', async () => {
    const parcel = new Parcel(await peerEndpointCert.calculateSubjectPrivateAddress(), endpointPdaCert, Buffer.from('hey'), { senderCaCertificateChain: [peerEndpointCert, privateGatewayCert] });
...
test('Messages by unauthorized senders should be refused', async () => {
    ...
    const unauthorizedSenderCertificate = reSerializeCertificate(await issueEndpointCertificate({
        issuerPrivateKey: keyPair.privateKey,
        subjectPublicKey: keyPair.publicKey,
        validityEndDate: TOMORROW,
        validityStartDate: ONE_SECOND_AGO,
    }));
    const parcel = new Parcel(await peerEndpointCert.calculateSubjectPrivateAddress(), unauthorizedSenderCertificate, Buffer.from('hey'), {
        creationDate: ONE_SECOND_AGO,
        senderCaCertificateChain: [peerEndpointCert, privateGatewayCert],
    });
