...
test('EnvelopedData should be decrypted after serialization', async () => {
    ...
    const envelopedDataDeserialized = EnvelopedData.deserialize(envelopedData.serialize());
    expectArrayBuffersToEqual(await envelopedDataDeserialized.decrypt(dhKeyPair.privateKey), plaintext);
});
