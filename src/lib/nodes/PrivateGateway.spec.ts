...
beforeAll(async () => {
  ...
  publicGatewayPrivateAddress = await getPrivateAddressFromIdentityKey(
    publicGatewayKeyPair.publicKey,
  );
...
describe('generateCCA', () => {
  ...
  test('Recipient should be public gateway', async () => {
    ...
    const ccaSerialized = await privateGateway.generateCCA(
  ...
  describe('Cargo Delivery Authorization', () => {
    ...
    describe('CDA issuer certificate', () => {
      test.todo('Certificate should be generated if none exists');

      test.todo('Certificate be generated if latest expires in 90 days');

      test.todo('Subject key should be that of private gateway');

      test.todo('Certificate should be self-issued');

      test.todo('Certificate should expire in 180 days when generated');

      test.todo('Certificate should be valid from 90 minutes in the past');
    });
