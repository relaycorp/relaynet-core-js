...
import { DetachedSignature, NONCE_SIGNATURE, PARCEL_DELIVERY } from './DetachedSignature';
...
describe('DetachedSignature', () => {
  ...
  describe('sign', () => {
    test('Plaintext should not be encapsulated', async () => {
      const signature = new DetachedSignature(OID_VALUE);
...
describe('DetachedSignature objects', () => {
  test('PARCEL_DELIVERY should use the right OID', () => {
    expect(PARCEL_DELIVERY.oid).toEqual(RELAYNET_OIDS.SIGNATURE.PARCEL_DELIVERY);
  });

  test('NONCE_SIGNATURE should use the right OID', () => {
    expect(NONCE_SIGNATURE.oid).toEqual(RELAYNET_OIDS.SIGNATURE.NONCE);
  });
});
